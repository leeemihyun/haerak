...
function search() {
 ...
 function jquery(search) {
	...
	var jsonParam={ clubname: searchInput};
	$.ajax({
		url : "clublistajax.do",
		dataType: "JSON",
		data : jsonParam,
		...
		success : function ( jsonObj ){
			...
			tbody+="<tr><td>"+
						ele.CLUB_NUM+"</td><td>"+
						ele.CATEGORY_NAME+"</td><td>"+
						"<a href='../club_info.do?club_Num="+ele.CLUB_NUM+"'>"+ele.CLUB_NAME+"</td><td></a>"+
						ele.USER_ID+"</td><td>"+
						ele.INPUT_DATE+"</td><td>"+
						"<button onclick='clubDelete(" + ele.CLUB_NUM + ")'>삭제</button></td></tr>";	
			} );//each
								
			if( cnt ==0 ){
				tbody="<tr><td colspan='6'>해당 조건의 모임이 없습니다.</td><tr>"
			}//end if
...
function clubDelete(clubNum) {
	$.ajax({
		url: "clubDelete.do",
		method: "POST",
		data: { club_num: clubNum },
		success: function(response) {
			// 삭제 성공한 후 수행할 동작을 여기에 작성하세요
			alert("모임이 삭제되었습니다!");
			jquery("");
		},
		error: function(xhr) {
...
<div id="wrap" > <!-- wrap( w: 800 x h:950 ) -->
		...
		<div id="container"><!-- container(w: 800 x h : 600 ) -->
			...
			<form action="clublistajax.do" name="frm" id="frm">
			<label  >모임조회</label>
			<i class="fa fa-search fa-lg" aria-hidden="true" ></i>
			<input type="text" class="text_search" name="clubname" id="id">
			<input type="button" value="확인"  class="btn_search" style="float: right; border-radius:5px" onclick="search()">
				...
				<td width="80">모임번호</td><td width="80">카테고리</td>
				<td width="400">모임명</td>
				<td width="80">회원명</td>
				<td width="120">작성일</td><td width="80">삭제</td>
				</tr>
				</thead>
